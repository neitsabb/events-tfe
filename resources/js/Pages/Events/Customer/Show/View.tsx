...
const artists = [
  {
    name: 'KAROL DHE',
    image:
      'https://res.cloudinary.com/shotgun/image/upload/ar_1:1,c_limit,f_auto,fl_lossy,q_auto,w_240/v1712868422/production/artworks/artists/karol_dhe',
  },
];
          ...
          <ul className="text-white/55">
            <li className="flex items-center gap-6">
              ...
              <div className="w-full text-muted-foreground  border-white/10 border-b py-4">
                <span className="text-orange">Samedi 11 août</span> de{' '}
                <b>13:00</b> à <b>20:00</b>
              </div>
            </li>
            <li className="flex items-center gap-6">
              <div>
              ...
              <div className="w-full text-muted-foreground  border-white/10 border-b py-4">
                <span className="text-white/90">Tous ensemble</span>{' '}
              </div>
            </li>
            <li className="flex items-center gap-6">
              <div>
              ...
              <div className="w-full text-muted-foreground py-4">
                <span className="text-white/90">
                  Chau. d'Houthem 15, 7780 Comines-Warneton, Belgique
                </span>{' '}
              </div>
            </li>
          </ul>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="w-fit max-w-[300px] gap-12 flex items-center justify-between">
              <div>
                ...
                <p className="text-white/55 text-xs">9 abonnés</p>
                <p className="text-white/75 text-sm">1 événement</p>
              </div>
              <Button
                variant="secondary"
...
const Section = ({
  ...
  return (
    <section className={cn('!max-w-3xl py-8', className)} {...props}>
      <h3 className="text-white/90 text-2xl font-semibold mb-4">{title}</h3>
      {children}
    </section>
  );
};
...
const Ticket = ({ ticket }: { ticket: Admission | Extra }) => {
  ...
  return (
    <div className="border-white/5 border-2 p-6 rounded-sm flex items-center justify-between">
      <div>
        <h4 className="text-white/90 text-lg font-semibold">{ticket.name}</h4>
        <p className="text-white/55 text-sm">{ticket.price} €</p>
      </div>
      <div>
        <Button
          className={cn('!px-4 !py-3 !h-auto uppercase !font-semibold')}
          disabled={ticket.sold === ticket.quantity}
          dangerouslySetInnerHTML={{
            __html:
              ticket.sold === ticket.quantity ? `&Eacute;puisé` : 'Acheter',
          }}
        />
      </div>
    </div>
